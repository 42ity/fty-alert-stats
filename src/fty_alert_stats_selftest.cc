/*  =========================================================================
    fty_alert_stats_selftest.c - run selftests
...
// Now built only with --enable-drafts, so even stable builds are hidden behind the flag
    { "fty_alert_stats_server", NULL, true, false, "fty_alert_stats_server_test" },
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_ALERT_STATS_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_alert_stats_private_selftest (verbose, item->subtest);
#endif // FTY_ALERT_STATS_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_alert_stats_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-alert-stats test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_ALERT_STATS_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_alert_stats_private_selftest (verbose, test->subtest);
#endif // FTY_ALERT_STATS_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
